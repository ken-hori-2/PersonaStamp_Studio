"""
Fish Audio TTS生成スクリプト（Streamlit用カスタマイズ版）

このスクリプトは、作成した音声クローンモデルを使用してTTSを実行します。
Streamlitアプリケーション用にカスタマイズされています。
model_id.txtへの依存を削除し、models.jsonでの管理に統一されています。
"""
...
def load_model_id_from_file(file_path: str = None) -> str:
    """
    models.jsonから最後に使用したモデルIDを読み込みます。
    ...
    model_id : str
        モデルID（見つからない場合はNone）
    """
    json_path = Path(__file__).parent.parent / "models.json"
