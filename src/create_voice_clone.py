"""
Fish Audio 音声クローンモデル作成スクリプト

このスクリプトは、sample_voice.wavから音声クローンモデルを作成します。
作成されたモデルIDは後でTTSに使用できます。
"""
...
def create_voice_clone_model(
    api_key: str,
    audio_file_path: str = "../examples/sample_voice.wav",
    ...
    api_key : str
        Fish AudioのAPIキー
    audio_file_path : str
        音声ファイルのパス（デフォルト: sample_voice.wav）
    model_title : str
        作成するモデルのタイトル
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*50)
        print("次のステップ:")
        print("1. model_id.txt に保存されたモデルIDを確認")
